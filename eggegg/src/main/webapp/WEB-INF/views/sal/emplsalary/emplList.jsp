			...
			<table>
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="radio" name="emplTenureAtt" value="ALL"> 전체&ensp;
							<input type="radio" name="emplTenureAtt" value checked> 재직자&ensp;
	...
	<table id="list-table">
		<thead>
			<tr>
				...
				<th>사원번호 <a class="sort">▼</a></th>
				<th>성명 <a class="sort">▼</a></th>
				<th>부서명 <a class="sort">▼</a></th>
				<th>직위/직급명 <a class="sort">▼</a></th>
				<th>입사일자 <a class="sort">▼</a></th>
				<th>재직구분 <a class="sort">▼</a></th>
			</tr>
		</thead>
		<tbody>
		</tbody>
...
<script>
	$(function(){
				...
				if(emplList.length > 0){
					$(emplList).each(function(idx, empl){
						...
						if(empl.retire != null){
							trTags.push(
								$("<tr>").append(
									$("<td class='emplListTd'>").append($("<input class='checkbox'>").attr("type", "checkbox")),
									$("<td>").append($("<a class='emplUpdate'>").html(empl.emplNo)),
									$("<td>").append($("<a class='emplUpdate'>").html(empl.emplNm)),
									$("<td>").html(deptNm),
									$("<td>").html(pstNm),
					...
					trTags.push(
						$("<tr>").html(
							$("<td>").attr("colspan", "7").html("조건에 맞는 사원이 없습니다.")	
						)
					);
				}
				tbody.append(trTags);
		...
		$(document).on("click", ".emplUpdate", function(){
			...
			$(this).parents("tr").find("td").css("background", "#FEFAE5");
			var nWidth = 1200;
			var nHeight = 1000;
			var curX = window.screenLeft;
			var curY = window.screenTop;
			...
			popup = window.open(
					"${ cPath }/empl/emplUpdate.do?emplNo=" + emplNo+"&sal=sal",
					"taxFree",
					"status=no, resizable=no, menubar=no,toolbar=no, location=no, scrollbars=no ,height=" + nHeight  + "px, width=" + nWidth  + "px, left="+ nLeft + "px, top="+ nTop+"px");
			popup.addEventListener('beforeunload', function(){
